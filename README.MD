...
- Веб-админка на `/`: разделы документов, загрузка файлов, чат, история тредов, настройки API/debug.
- REST API на FastAPI: health, разделы, документы, RAG-чат, экспорт ответа, треды чата, audit log.
...
- Опциональный Polza/OpenAI-compatible LLM через `POLZA_*`, `ALLOW_LLM_EGRESS` и allowlist моделей.
