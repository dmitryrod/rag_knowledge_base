...
На вкладках **«Чат»** или **«Тесты»** в ответе ассистента в русском тексте появляются «кракозябры» или символ замены Unicode **U+FFFD** (в UI часто пара «битых» символов внутри слова, например искажённое `твердостью`).

## Причина

- **Уже испорченный индекс Chroma:** чанки попали в базу до исправления ingest (старый `decode(..., errors="replace")`) или текст извлечён из PDF/другого формата с потерей байтов — в сегменте контекста уже есть `\ufffd`, модель копирует его в ответ.
- Реже: **глюк/ограничение модели** при копировании длинных кириллических фрагментов (проверь, есть ли `\ufffd` в поле `debug.retrieval_encoding`).

## Шаги

1. Включи отладку: в **Настройках** переключатель отладки (заголовок `X-Debug: 1` на запросы чата) или для прогона **«Тесты»** поставь `debug: true` в `POST /v1/rag-test/run`.
2. В ответе чата открой поле `debug` → **`retrieval_encoding`**. Если `count > 0`, в списке `items` видны `chunk_id` и `filename` чанков с U+FFFD — **удали документ и загрузи снова** (или перезалей исходник в UTF-8 / исправь PDF-источник). Один символ `\ufffd` **нельзя** восстановить без исходного файла.
3. Если `retrieval_encoding.count == 0`, а `\ufffd` только в ответе — смени модель/повтори запрос; при необходимости сократи контекст в prompt.
4. См. также запись про ingest и перезаливку в `app/docs/CHANGELOG.md` (раздел про `.txt` / U+FFFD).

## Как проверить

`python -m pytest app/tests/test_api.py::test_chat_thread_messages_preserve_utf8_no_replacement_char app/tests/test_rag_test_api.py::test_rag_test_run_and_compare_preserve_utf8_no_replacement_char app/tests/test_chat_service.py::test_replacement_char_report_detects_u_fffd -q` — цепочка API и отчёт по чанкам с U+FFFD.
...
Строгий system-prompt или сохранённый override может провоцировать ложное «не найдено», хотя релевантные чанки в контексте есть. С версии 2026-04-26 чат спасает такие случаи локально: если вопрос и чанки лексически совпадают, возвращается extractive fallback с цитатами; если совпадения нет, цитаты не подставляются, чтобы не маскировать плохой retrieval/scope.

## Шаги

1. Включи **Debug** в настройках веб-админки: в ответе чата поле `debug` (при `X-Debug: 1`) покажет `collection_ids`, top chunks и distance — видно, пустой ли scope или шумный retrieval.  
2. Если документ лежит во **вложенном** разделе, а в чате выбран только родитель — с версии 2026-04-26 поиск автоматически включает поддерево разделов; обнови сервер.  
3. После **переноса** файла DnD создай новый чат или проверь «Выбор документов»: scope должен совпадать с разделом, где документ сейчас (UI подменяет выбранный id при переносе). Старый тред с узким scope по пустой коллекции закономерно даёт 0 чанков.  
4. Сбрось серверные overrides: **Настройки** → «Сбросить overrides чата» или `DELETE /v1/rag-test/main-chat-profile` — убирает случайный `distance_threshold` / кастомный `system_prompt` из A/B.  
5. Проверь `POLZA_CHAT_MODEL` и при заданном `POLZA_CHAT_MODEL_ALLOWLIST` — модель должна входить в список.

## Как проверить

`python -m pytest app/tests/test_chat_service.py -q` — при явном «не найдено» от LLM и пустых citations релевантный retrieval получает extractive fallback с цитатами; нерелевантный retrieval по-прежнему не маскируется fallback-цитатами.

---

## Симптом

В чате «НЕ НАЙДЕНО В БАЗЕ» или нерелевантные цитаты, хотя документ в дереве есть; после перетаскивания файлов между папками стало хуже.

## Причина

До фикса поиск шёл только по **точным** id выбранных разделов: документ во вложенной папке не попадал в Chroma-запрос, если был выбран только родитель. Плюс сохранённые **rag_runtime_settings** (`distance_threshold`, свой prompt) и устаревший **scope** треда после переноса.

## Шаги

1. Обнови код и перезапусти API; проверь `GET /v1/rag-test/main-chat-profile` — при необходимости сбрось overrides.  
2. В чате включи «Все разделы» или выбери раздел, где документ **сейчас** (после DnD список чатов обновится).  
3. `python -m pytest app/tests/test_api.py::test_chat_rag_parent_scope_finds_document_in_child_section -q` — контроль поддерева.

## Как проверить

Вопрос по содержимому документа во вложенном разделе при выборе только родителя в «Выбор документов» возвращает `chunks_considered >= 1`.
...
1. С хоста: `nslookup` / `ping` к хосту из `POLZA_BASE_URL` (например `polza.ai`) — сравни с тем же **внутри контейнера**: `docker exec -it rag_knowledge_base-knowledge-api getent hosts polza.ai` (или `nslookup` если есть в образе).
...
В ответе API или в UI теста RAG: `The handshake operation timed out` / `_ssl.c:...` / ошибка в «query» при первом поиске по Chroma (чат, «Тесты»).

## Причина

Chroma по умолчанию векторизует запрос моделью **ONNX all-MiniLM-L6-v2**; при пустом кэше **скачивает** архив по HTTPS с `chroma-onnx-models.s3.amazonaws.com`. Тайм-аут SSL — нет маршрута до S3, фильтр, прокси, плохой DNS, «тяжёлый» с первого раза канал, или контейнер без выхода в интернет / без DNS (как в `CHANGELOG` для pip/Docker).

## Шаги

1. С машины, где крутится API: дождаться успешной загрузки (повторить запрос) или проверить `curl -I` / браузером доступ к `https://chroma-onnx-models.s3.amazonaws.com/`.  
2. Закэшировать модель: после первой удачной выдачи каталог `~/.cache/chroma` (в Docker — `root` или `APPUSER`, смонтировать volume на этот путь между пересборками).  
3. Docker: в `docker-compose` добавить `dns` (`8.8.8.8`) при типичных `Errno -3` / нестабильном резолве.  
4. API теперь отдаёт **502** с развёрнутым `detail` вместо сырого OpenSSL; ищи в теле ответа подсказку про S3 и кэш.

## Как проверить

Повторный `POST` чата/теста после появления кэша — без повторного скачивания запросы должны проходить (если проблема была только в первом тайм-ауте).
