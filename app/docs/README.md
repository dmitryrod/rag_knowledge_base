...
| `TODO.md` | Живой бэклог: что доделать до гейтов и техдолг (сверка с кодом). |
...
4. Запуск: `knowledge-api` (скрипт из пакета) **или** `uvicorn app.main:app --host 0.0.0.0 --port 8000` с `PYTHONPATH` на корень workspace (иначе импорт `app` не найдётся). В браузере открывай **`/`** — веб-админка в стиле ChatGPT: табы «Документы» / **«Чат»** / **«Тесты»** (A/B: формы профилей + общий выбор разделов/документов для `scope`, API `/v1/rag-test/...`, избранные снимки в `APP_DATA_DIR/tests_favorite` через `/v1/rag-test/favorites`); настройки по **⚙** (тема интерфейса: `localStorage` `knowledge_theme`, значения `dark` / `light` / `blue` / `pink` / `gray`); на «Чат» слева список диалогов с серверной историей (`/v1/chat/threads/...`); на «Тесты» слева список избранных по ID. `X-API-Key` при `auth_configured`; **Debug** — `X-Debug: 1` (см. `APP_ALLOW_CLIENT_DEBUG` в `app/.env.example`). Swagger: `/docs`. Порты: `PORT` / `APP_PORT`, в Docker `APP_EXPOSED_PORT` в логе.
